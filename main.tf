provider "alicloud" {
  ...
  configuration_source    = "terraform-alicloud-modules/emr-druid"
}
...
resource "alicloud_ram_role" "default" {
  ...
  name        = var.ram_role_name == "" ? substr("tf-ram-role-for-druid-${replace(random_uuid.this.result, "-", "")}", 0, 32) : var.ram_role_name
  document    = <<EOF
    {
        "Statement": [
        {
